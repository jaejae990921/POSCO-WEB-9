...
const cookieConfig = {
    ...
    // 자바스크립트에서 document.cookie를 이용하여 쿠키에 접속하는 것을 차단
    // maxAge : 쿠키 수명 (밀리초 단위)
    // expires : 쿠키 만료일 (GMT 시간)
...
// 생성한 쿠키 확인
app.get('/getCooke', (req, res) => {
    res.send(req.signedCookies); // 암호화된 쿠키, signedCokies에 저장되어 있으므로 req.signedCookies로 확인
})
